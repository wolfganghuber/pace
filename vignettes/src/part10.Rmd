...
```{r}
pheatmap( cor(exprs(e)), symm=TRUE, cluster_rows = FALSE, cluster_cols = FALSE, 
...
```{r}
df  <- exprs(lpdAll)[c(ib, ev, se), lpdAll$Diagnosis=="CLL"] %>%
...
for (i in 1: max(which(fData(lpdCLL)$type=="viab"))) {
  ...
  fit <- aov(exprs(lpdCLL)[i, rownames(sel)] ~ sel$group)
  p[i] <- summary(fit)[[1]][["Pr(>F)"]][1]
  
  
  calc_p = function(clmn) {
    p.adjust(
      t.test(exprs(lpdCLL)[i, rownames(sel)] ~ sel[,clmn],
             alternative = c("two.sided") )$p.value, "BH" )
  }
  
  calc_d = function(clmn) {
    diff(
      t.test(exprs(lpdCLL)[i, rownames(sel)] ~ sel[,clmn])$estimate,
...
groupMean = function(gr) {
  rowMeans(exprs(lpdCLL)[rownames(ps45), rownames(sel)[sel$group==gr]])
}
...
beeDrug <- function(xDrug) {
   ...
   beeswarm(
     exprs(lpdCLL)[xDrug, rownames(sel)] ~ groups$group,
     axes=FALSE, cex.lab=1.5, ylab="% viability", xlab="", pch = 16,
     pwcol=sel$col, cex=1.5,
     ylim=c(min( exprs(lpdCLL)[xDrug, rownames(sel)] ) - 0.05, 1.2) )
 
   boxplot(exprs(lpdCLL)[xDrug, rownames(sel)] ~ groups$group,  add = T,
           col="#0000ff22", cex.lab=2) 
   
   mtext(side=3, outer=F, line=0, 
       paste0(giveDrugLabel3(xDrug) ), cex=3) 
}
...
```{r}
genes <- data.frame(
  t(exprs(lpdCLL)[fData(lpdCLL)$type %in% c("gen", "IGHV"), rownames(sel)]),
...
load(system.file("extdata","gexGroups.RData", package="pace"))
```
...
```{r echo=TRUE, eval=TRUE}
load(system.file("extdata","genSymbols.RData", package="pace"))
```
...
c <- cor.test( exprs(lpdCLL)[drug, patsel], vsd[gen, patsel] )
...
plot(vsd[gen, patsel], exprs(lpdCLL)[drug, patsel],
     ...
     cex=1.2)
abline(lm(exprs(lpdCLL)[drug, patsel] ~ vsd[gen, patsel]))
```
